...
argument-hint: "[--strict|--nuclear] [--draft] [--review-with <agent>[,<agent>...]] [--review-iterations <n>] [--review-stop-on-findings|--review-stop-on-clean] [--reviewer-applies] [--issues|--no-issues] [--issues-label <name>] [PR-URL | base-branch]"
---
...
- `codex` | `agy` | `claude` — invoke the local-agent review loop. The CLI runs its installed review command headless (`/do:review` under claude, `/do-review` under agy) against the same scope this invocation reviewed:
  ...
  - Note on `codex` in PR mode: the local-agent loop's review-only path uses `codex --sandbox danger-full-access review --base "$BASE_BRANCH"`, which expects a git ref and does not accept a PR URL. The apply-mode path (`codex --sandbox danger-full-access -a never exec "$CODEX_APPLY_PROMPT"`) does not rescue this either — `CODEX_APPLY_PROMPT` is templated as `"Review the diff from $BASE_BRANCH to HEAD in this repo ..."` and explicitly references `$BASE_BRANCH` as a git ref the repo can resolve, plus assumes the PR branch is checked out locally so codex can read the working tree. **In PR mode the codex pass is therefore skipped regardless of `--reviewer-applies`**, with the printed message `Skipping codex pass in PR mode: neither codex review --base nor codex exec's apply-mode prompt accept a PR URL (both require a local git ref and a checked-out branch). Use --review-with codex against a local branch instead.` The skip is recorded in the per-pass table as status `skipped`, treated like a non-fix inconclusive for `{OVERALL_STATUS}` purposes. The `agy` and `claude` paths invoke slashdo's installed review command (`/do-review` under agy, `/do:review` under claude) and accept either form. **The `ollama` pass is likewise skipped in PR mode**: the Ollama loop builds its prompt from a local `git diff $BASE_BRANCH...HEAD`, which needs a local git ref and a checked-out branch — a PR URL won't resolve. Print `Skipping ollama pass in PR mode: the Ollama loop reviews a local git diff and cannot resolve a PR URL. Use --review-with ollama against a local branch instead.` and record the pass as `skipped` (treated as inconclusive for `{OVERALL_STATUS}`).
