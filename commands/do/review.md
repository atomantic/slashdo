...
**Review is judgment-heavy, and `heavy` inherits rather than upgrades** — so these agents are only as strong as the session you launched from. If this session is not on your strongest model, **say so once before spawning** (`review agents inherit this session's model (<model>); a stronger session finds more`) and continue. Don't block on it, and don't pin a model slug to force an upgrade: a hardcoded name fights an org's pinned version and is meaningless on the other CLIs slashdo installs into.
...
- `codex` | `agy` | `claude` | `grok` — invoke the local-agent review loop. The CLI runs a self-contained single-agent review prompt headless (codex uses its built-in `codex review`) — not the `/do:review` multi-sub-agent skill, which hangs under a print-mode/headless invocation — against the same scope this invocation reviewed:
