...
- `codex` | `agy` | `claude` | `grok` | `cursor` — invoke the local-agent review loop. The CLI runs a self-contained single-agent review prompt headless (codex uses its built-in `codex review`) — not the `/do:review` multi-sub-agent skill, which hangs under a print-mode/headless invocation — against the same scope this invocation reviewed:
