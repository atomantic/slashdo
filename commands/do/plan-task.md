...
- **`--enhance-with <list>`** — after the draft is written, route it through an
  **ordered pipeline of enhancement agents** that sharpen it before the approval gate
  ...
  `agy` (aliases `gemini`/`antigravity`), `grok`, `pi`, and `cursor` (alias `cursor-agent`)** — the agentic CLIs that take a
  ...
  selectors may contain spaces and parens (e.g. `--enhance-with 'agy[Gemini 3.8
  ...
  `agy[Gemini` as the value and leak `3.8 Flash (High)]` into the task
  ...
  with `Unknown --enhance-with value: {value}. Use one of: codex, claude, agy, grok, pi, cursor.`
