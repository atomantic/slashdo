...
- The selected canonical file path MUST NOT exist. If the candidate exists, try sibling names by adding `-1`, `-2`, ... `-100` to its basename stem before `.md`; choose the first path that does not exist, and abort before scanning if all candidates are taken. This rule applies to the default date-based path and an explicit `--report-path` alike; never overwrite an earlier report.
...
- **`--report-path <path>`**: where to write the markdown report. Default candidate: `~/.claude/scans/{basename}-{YYYY-MM-DD}.md`; if it exists, choose the first free `-1` through `-100` suffix so repeated scans keep prior reports.
...
- Set the default candidate to `~/.claude/scans/{BASENAME}-{SCAN_DATE}.md`, or use the candidate from `--report-path` when supplied. Create `~/.claude/scans/` only for the default path if it does not exist (the ONE directory the scan is allowed to create).
- Apply Invariant **I9** to either candidate (extension, canonical parent, allowed root, and collision-safe selection). On failure, abort. Record the selected free path as `REPORT_PATH` before scanning.
