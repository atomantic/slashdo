...
2. **Check for existing code review and decide which reviewer (if any) to request** (only if `is_fork_pr=false`): Query the PR's review requests and recent reviews:
   ...
   ```
   Note whether **any** completed review exists (from a copilot bot, a human, or another bot) — call this `HAS_EXISTING_REVIEW` — and specifically whether a `copilot-pull-request-reviewer` review exists or is pending — call this `HAS_COPILOT_REVIEW`. Then dispatch on `REVIEW_AGENTS`:
