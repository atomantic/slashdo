...
- **`@<login>`-only values resolve to no reviewer**: if dropping `@<login>` entries per the bullet above leaves `REVIEW_AGENTS` empty, the emptiness stands — set `REVIEW_AGENTS=[]` and run the no-reviewer path (resolve existing threads only). Do **not** fall through to the saved-defaults step when the user explicitly typed `--review-with` this run; an explicit flag already wins over a saved default, per the precedence rule below.
- If `REVIEW_AGENTS` names a **local CLI** (`codex`/`agy`/`claude`/`grok`, in any combination), rpr requests each such reviewer via the **local-agent review loop** (`lib/local-agent-review-loop.md`) against the PR branch. An `ollama` entry instead goes through the **Ollama review loop** (`lib/ollama-review-loop.md`) — it requires the PR branch checked out locally, since the loop reviews a local `git diff`. A `copilot` entry, if you asked for one, goes through the Copilot request/poll path below.
...
8. **Decide whether to loop** (only if `is_fork_pr=false`): After pushing fixes, evaluate whether another review round is worth running. **Skip for fork-to-upstream PRs.**
   ...
   **Re-request gate (which reviewer, if any):**
   - **If `REVIEW_AGENTS` is empty, there is no reviewer to re-request** — do not summon one. Skip the worthiness evaluation and proceed to step 9.
   - **Only re-request a Copilot review if Copilot is the reviewer actually in play** — i.e. `REVIEW_AGENTS` contains `copilot` **and** the threads you just resolved came from a Copilot review (`HAS_COPILOT_REVIEW`). If the round resolved only non-Copilot threads (e.g. a human review), do NOT request a Copilot review — resolve and proceed to step 9. (rpr summons Copilot only when Copilot is already reviewing.) A `copilot~max=<n>` cap bounds this re-request loop under the same accounting as every other entry (below): rpr's Copilot loop is bespoke rather than a dispatch into `lib/copilot-review-loop.md`, so nothing else enforces the budget — count each requested Copilot round and stop re-requesting once `n` is spent.
   - If `REVIEW_AGENTS` names a **local CLI** (`codex`/`agy`/`claude`/`grok`), "another round" means re-running that agent's local-agent review loop — not a Copilot request. That loop manages the cap *within* one dispatch, so typically one pass suffices; loop again only if you made substantive fixes that warrant a fresh local pass — and only if the entry has budget left (see the cap-accounting rule below).
   - If `REVIEW_AGENTS` names `ollama`, "another round" means re-running the **Ollama review loop** (`lib/ollama-review-loop.md`) against the locally checked-out PR branch, forwarding `{OLLAMA_MODEL}` — not a Copilot request. Its cap is `{MAX_ITERATIONS}` (the built-in `3` unless a `~max=<n>` moved it), so loop again only if the last round made substantive fixes **and** the entry has budget left (see below).
   - **A per-entry `~max=<n>` is a total budget, not a per-dispatch one — for every reviewer type, including `copilot` and `@<login>`.** Each dispatch of an inner loop enforces the cap only within that dispatch, so re-dispatching the same entry here would hand it a fresh `n` every outer round and let `--review-with ollama~max=1` run an unbounded number of review-and-fix cycles — the exact opposite of what the flag buys. Track each entry's **rounds spent so far** across this outer loop (sum the iterations its inner loop reported on every dispatch) and, for an entry whose cap was explicitly configured (`{MAX_EXPLICIT}=true`, `n ≥ 1`), stop re-dispatching it once the total reaches `n` — forwarding the *remaining* budget (`n - spent`), not `n`, on any subsequent dispatch. An entry on its built-in default cap, or on `~max=0` (unlimited), keeps the historical behavior: the worthiness evaluation below is what stops it. `copilot` has **no** built-in per-entry cap in rpr (its loop is bespoke, and rpr accepts no `--review-iterations`), so an uncapped `copilot` entry is likewise governed only by the worthiness evaluation — exactly as before this release.
...
When `REVIEW_AGENTS` names `ollama`, step 2 (and the step-8 re-request) runs the Ollama review loop against the locally checked-out PR branch. Pass `{OLLAMA_MODEL}` (empty = auto-select), the PR branch (`headRefName`) checked out locally, the base branch (`baseRefName`), and the project `{BUILD_CMD}`. The loop is always review-only (Ollama is non-agentic): it emits findings, the orchestrator applies them, and the main thread verifies build + tests before pushing.

!`cat ~/.claude/lib/ollama-review-loop.md`

## Requesting GitHub Copilot Code Review (legacy — only when `copilot` is in `REVIEW_AGENTS`)

This path runs **only** when you asked for `copilot` explicitly (typed flag or saved default). rpr never summons Copilot on its own.
