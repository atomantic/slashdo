...
8. **Decide whether to loop** (only if `is_fork_pr=false`): After pushing fixes, evaluate whether another review round is worth running. **Skip for fork-to-upstream PRs.**
   ...
   **Re-request gate (which reviewer, if any):**
   ...
   - If `REVIEW_AGENTS` names a **local CLI** (`codex`/`agy`/`claude`/`grok`), "another round" means re-running that agent's local-agent review loop — not a Copilot request. The local-agent loop manages its own fixed iteration cap, so typically one pass suffices; loop again only if you made substantive fixes that warrant a fresh local pass.
