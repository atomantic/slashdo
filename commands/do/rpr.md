...
2. **Check for existing code review and decide which reviewer (if any) to request** (only if `is_fork_pr=false`): Query the PR's review requests and recent reviews:
   ...
   - **If `REVIEW_AGENTS` contains `copilot` (including the default):** a `copilot~max=<n>` cap applies to this path too — see the cap-accounting rule in step 8. rpr accepts no `--review-iterations`, so `~max` is the only budget signal here, and each Copilot round below counts against it.
