...
Parse `$ARGUMENTS` for `--reviewer-applies` (boolean): record `REVIEWER_APPLIES=true`/`false` (default `false`). Forwarded to any local-agent review loop; no effect on the Copilot path (warn if combined with a copilot-only list) or the ollama path (Ollama is non-agentic — always review-only).
...
2. **Check for existing code review and decide which reviewer (if any) to request** (only if `is_fork_pr=false`): Query the PR's review requests and recent reviews:
   ...
   - **If `REVIEW_AGENTS` contains a local CLI (`codex`/`agy`/`claude`/`grok`/`pi`/`cursor`/`opencode`):** run the **local-agent review loop** (`lib/local-agent-review-loop.md`, "Local-Agent Review Loop" below) for each such agent against the PR branch, forwarding `REVIEWER_APPLIES` and that entry's `{REVIEW_MODEL}`, `{REVIEW_EFFORT}`, and `{MAX_ITERATIONS}` / `{MAX_EXPLICIT}` (from `~max=<n>`; the built-in `3` / `false` when none). This produces findings (and, in reviewer-applies mode, fixes) locally — it does **not** request a Copilot cloud review. Then proceed to step 3 to resolve any pre-existing threads as well.
