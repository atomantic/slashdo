...
- Update PLAN.md/tracker exactly per [lib/better-cleanup.md](../../lib/better-cleanup.md) step 5 — **PLAN.md only outside issue mode** (flip `- [ ]` → `- [x]`, preserving the `[<slug>]` ID; note skipped findings with reasons); in issue mode, update/report tracker records instead, per [lib/better-issue-mode.md](../../lib/better-issue-mode.md). Stage any PLAN.md changes so Phase B's commit picks them up.
