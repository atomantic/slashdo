...
2. **Commit and push** — commit any remaining staged changes (e.g., the PLAN.md update), then run `do:pr`'s "Commit and Push" step verbatim: rebase the branch onto the latest `origin/{default_branch}` (`git fetch origin {default_branch} && git rebase origin/{default_branch}`, aborting and surfacing conflicts rather than guessing) so the reviewers below diff against a current base, then `git push -u origin {current_branch}` (`--force-with-lease` if the rebase rewrote pushed history)
