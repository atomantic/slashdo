...
- **`<slug>` / `#<issue>`** — claim THAT specific item instead of auto-picking (cherry-pick out of order). PLAN.md mode: a slug that must already exist as a `- [ ]` line (this command never *assigns* IDs — that's `/do:replan`'s job). Issues mode: an issue number, bare (`123`) or `#`-prefixed (`#123`, strip the `#`); must be open. An explicit number is a deliberate cherry-pick that **bypasses every auto-pick skip** — it can claim a parking-labelled issue (`future`/`blocked`/`epic`/…) or, when a label filter is active, an issue outside that filter; state it when you do.
...
1. **List candidates** — open issues, oldest-first, **across all labels by default** (`gh issue list` never returns pull requests, so PRs are excluded automatically). **There is no author filter and no required label.** Auto-pick claims any open issue regardless of who filed it or what label it carries — the guards against claiming the wrong thing are the parking-label skip (step 3) and the in-flight/assigned checks, *not* a gating label. Only when a label filter is active (`LABEL_FILTER` set via `--issues-label` or a saved `issues-label` default) is the candidate set narrowed to that one curated label:
   ```bash
...
3. **Pick the target issue:**
   - **With argument** — the issue number (strip `#`). Verify open and NOT in flight. A named number is an **explicit override**: it claims even an issue that auto-pick would skip — one carrying a parking label, or (when `LABEL_FILTER` is active) one outside the curated label. Proceed, but state plainly when you're overriding a skip (e.g. "claiming `future`-labelled #123 by explicit request") so the choice is visible. If any other check fails (closed, in flight), print why and stop.
   - **Without argument** — pick the FIRST (oldest) candidate NOT in flight, NOT already assigned, and NOT carrying a parking label (`blocked`, `needs-input`, `wontfix`, `discussion`, `future`, `epic`, or any repo-specific parking label — skip and note it). Because auto-pick is label-agnostic by default, the parking-label skip is now the **primary** guard against claiming parked or umbrella work — `epic` is included so multi-issue umbrellas (which aren't single claimable units) aren't auto-claimed. An explicit `#num` can still claim a parking-labelled issue; auto-pick never surfaces one.
...
- **Empty pick is not a failure.** Everything in flight / drifted / NEEDS_INPUT (PLAN.md), or every open issue in flight / assigned / parking-labelled — or, when `--issues-label` is active, no open issue carrying that label (issues) — is a healthy queue — exit clean and say so.
