...
**A1e — Vet each named issue; no picker, no substitutions.** The list is a deliberate cherry-pick, so it **bypasses the auto-pick skips exactly as a single explicit `#<num>` does**: parking labels (`future`/`blocked`/`discussion`/…), an active `LABEL_FILTER`, and an open declared blocker that was **never named in the list** are all overridden — state each override as you apply it (e.g. `claiming future-labelled #123 by explicit request`). **A blocker that *was* named and then dropped below is a different case**: it is still open and still blocks its dependent, so hold that dependent per Phase B's wave rule — don't re-read it as an out-of-list override. Do still check each named issue, in this order:
...
**A2e — Order the batch and split it into waves.** Keep the **user's order** — a named list is a stated preference, so don't re-sort it by priority — then:
   ...
   - **Chunk into waves of at most `SWARM_N`,** preserving that order **and the two placement constraints above** — which outrank the chunk size: a member deferred behind a predecessor or an overlap peer must land in a **strictly later** wave than it, so a wave may come out smaller than `SWARM_N`. Only when **nothing was deferred** and `len(TARGETS) <= SWARM_N` is this a single wave (the common case). State the plan up front: `9 issues named, 3 at a time — 3 waves (≈3× the tokens of a single /do:next per wave)`.
