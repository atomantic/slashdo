...
- **Saved defaults.** With neither `--issues` nor `--no-issues`, resolve `ISSUE_MODE` from the saved `issues` default. Load it as follows: (1) read the global config at `~/.claude/.slashdo-config.json` and extract its `.defaults` object; (2) read the per-project config at `<repo-root>/.slashdo.json` (skip silently if not in a git repo) and extract its `.defaults` object; (3) merge them, project-over-global — `EFFECTIVE_DEFAULTS = { ...GLOBAL_DEFAULTS, ...PROJECT_DEFAULTS }`. For `ISSUE_MODE`, take `EFFECTIVE_DEFAULTS.issues` if present, else built-in `false`. Likewise resolve `PLAN_LABEL` from `EFFECTIVE_DEFAULTS["issues-label"]` (default `plan`), `SELF_MODE` from `EFFECTIVE_DEFAULTS.self` (default `false`), `COLLAB_MODE` from `EFFECTIVE_DEFAULTS.collaborators` (default `false`), and `TRUSTED_AUTHORS` from `EFFECTIVE_DEFAULTS["trusted-authors"]` (default empty, tombstone `none` means explicitly no extra authors). A typed flag always wins over its saved default. Resolve only these five here — the review flags pass through to `/do:pr`, which resolves its own defaults. **`--model` / `--effort` have no saved default by design** — a forgotten saved narrowing is indistinguishable from an empty backlog; they apply only when typed. The Phase 1 auto-redirect applies independently: a repo with no PLAN.md / the issue-mode stub switches to issue mode even with no saved default or a saved `issues=false`; only an **explicit** `--no-issues` on the command line wins over it (Phase 1).
