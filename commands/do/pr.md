...
- If any value is not in the accepted set, abort with a usage error: `Unknown --review-with value: {value}. Use one of: codex, agy, claude, grok, ollama, copilot, @<login> (each optionally suffixed ~opt, ~max=<n>, and/or ~effort=<level>).`
