...
4. **Merge once CI is green** — GitHub (`gh`):
   - **Resolve `{LINKED_WORKTREE}` first** — `/do:pr` is routinely invoked from inside a `git worktree` (by `/do:next`, `/do:pr-better`, and the claim flows), and that changes which merge flags are safe:
     ...
     `--delete-branch` deletes the **local** branch as well as the remote one, and to do that `gh` first checks out the default branch — which fails in a linked worktree (`fatal: '{default_branch}' is already used by worktree at …`, because the parent repo already has it checked out). `gh` then **exits non-zero even though the merge itself succeeded**, so the merge reads as a failure and any `||` fallback chain wrapped around it fires against an already-merged PR. **When `LINKED_WORKTREE=1`, omit `--delete-branch` from every merge command below**; after a *completed* merge delete the remote branch explicitly with `git push "$PUSH_REMOTE" --delete "$PUSH_BRANCH"` — the same config-derived remote and ref the push step above resolved (`branch.$BR.remote` / `branch.$BR.merge`), never a hardcoded `origin` plus the local branch name, which on a branch whose upstream is `upstream/feature-x` or `origin/pr-123-head` would delete an unrelated remote branch; skip the delete when `PUSH_REMOTE` is empty or `.`. It needs no local checkout, and leave the local worktree + branch to the caller's cleanup phase, which removes them from the parent repo where that works. When the merge is only *queued* (auto-merge), delete nothing — the branches outlive this session, and the repo's "automatically delete head branches" setting or the caller's cleanup owns them.
