...
- Dedupe preserving first-occurrence order (compare on the normalized slug — for `ollama` the bracket suffix is part of the identity, so `ollama[a]` and `ollama[b]` are distinct while two bare `ollama`s collapse; for `@<login>` the login is the identity, compared lowercased, so `@Octocat`/`@octocat` collapse while distinct logins stay separate; the `~opt` marker is **not** part of the identity, so `ollama~opt` collapses with `ollama` — the survivor is optional if any collapsed occurrence had `~opt`); if duplicates were dropped, print: `Note: deduped --review-with list to {final list}.`
