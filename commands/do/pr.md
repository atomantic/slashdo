...
- **Precondition for any cross-phase skip**: every entry in `PR_SIDE_AGENTS` must appear *after* every entry in `LOCAL_AGENTS` in the original `--review-with` order (i.e. the user's list was already naturally "local reviewers, then PR-side reviewers" — e.g. `codex,copilot` or `codex,agy,@octocat`, not `@octocat,codex` or `copilot,agy`). If any `PR_SIDE_AGENTS` entry precedes any `LOCAL_AGENTS` entry in the original order, the cross-phase skip below does **not** apply — always run `PR_SIDE_AGENTS` in full when non-empty, exactly as before this optimization existed, so that earlier-listed reviewer is still requested.
- When the precondition holds, check whether the pre-PR local phase already satisfied `{REVIEW_STOP_MODE}`:
  - `REVIEW_STOP_MODE=on-clean`: if `LOCAL_OVERALL_STATUS` is `clean` or `partial` (a clean verdict was reached — `partial` means the local phase's own wrapper invocation already short-circuited on a clean pass; `clean` means its whole list ran and was clean), skip `PR_SIDE_AGENTS` entirely: set `PR_SIDE_OVERALL_STATUS=clean` (skipped — already satisfied by the local phase) and continue to "Compute OVERALL_STATUS".
