...
- Accepted values per slot: `codex`, `agy` (aliases `gemini` / `antigravity` — all run the Antigravity CLI's `agy` binary), `claude`, `grok`, `cursor` (alias `cursor-agent` — the Cursor Agent CLI), `opencode` (aliases `zen` / `opencode-zen` — the OpenCode CLI), `ollama`, `copilot` (**legacy** — GitHub's cloud Copilot review; still supported when you name it, never selected implicitly), or an arbitrary GitHub login `@<login>`
...
- `codex`, `claude`, `agy`, `grok`, `cursor`, and `opencode` likewise accept a `<agent>[<model>]` bracket to pin the reviewing model — e.g. `codex[o3]`, `claude[claude-opus-4-8]`, `agy[Gemini 3.5 Flash (High)]`, `grok[grok-code-fast-1]`, `cursor[gpt-5]`, `opencode[muse-1.3]`. Strip the bracket into a per-entry `REVIEW_MODEL` (empty → the reviewer's built-in default: the codex/claude/grok/cursor CLI default, or agy/opencode's pinned default) and keep the base slug. The bracket value is free-form (may contain spaces/parens — validate shape, not an allowlist); `copilot` and `@<login>` take **no** model bracket. When a token omits the bracket, a saved `review-models` default (see `/do:config`) supplies the model — the explicit bracket always wins. Full mechanics in `lib/multi-reviewer-loop.md`.
...
- The value may be a single agent or a **comma-separated, ordered list** (e.g. `--review-with codex,agy,copilot`). Split on `,`, trim whitespace around each slug. Normalize `gemini`/`antigravity` → `agy`, `cursor-agent` → `cursor`, `zen`/`opencode-zen` → `opencode`.
...
- Dedupe preserving first-occurrence order (compare on the normalized slug — for a model-taking agent (`codex`/`claude`/`agy`/`grok`/`cursor`/`opencode`/`ollama`) the `[<model>]` bracket suffix is part of the identity, so `codex[a]` and `codex[b]` are distinct while two bare `ollama`s collapse; for `@<login>` the login is the identity, compared lowercased, so `@Octocat`/`@octocat` collapse while distinct logins stay separate; no `~` suffix is part of the identity, so `ollama~opt`, `ollama~max=2`, `ollama~effort=high` all collapse with `ollama` — the survivor is optional if any collapsed occurrence had `~opt`, and takes its cap and effort level from the first occurrence that carried them); if duplicates were dropped, print: `Note: deduped --review-with list to {final list}.`
- If any value is not in the accepted set, abort with a usage error: `Unknown --review-with value: {value}. Use one of: codex, agy, claude, grok, cursor, opencode, ollama, copilot, @<login> (each optionally suffixed ~opt, ~max=<n>, and/or ~effort=<level>).`
...
- This flag has no effect on local-agent reviewers (`codex`/`agy`/`claude`/`grok`/`cursor`/`opencode`) or `ollama`; they keep their own fixed iteration caps. To move *their* caps — or to give each reviewer a different budget in one run — use the per-entry `--review-with <agent>~max=<n>` suffix, which overrides this flag for the entry that carries it.
...
2. Run `git diff {REVIEW_BASE}...{current_branch}` to get the list of changed files — the base resolved in "Commit and Push" above, which is `origin/{default_branch}` on the linked-worktree fallback. Diffing the un-advanced local ref there would enumerate every file that landed on the default branch outside this branch, and step 3 reads each of them in full.
...
- `LOCAL_AGENTS` — every entry that is neither `copilot` nor an `@<login>` GitHub reviewer: `codex`, `agy`, `claude`, `grok`, `cursor`, `opencode`, `ollama[…]`. These review the working tree locally and do not need a PR to exist.
...
4. **Merge once CI is green** — GitHub (`gh`):
   - **Resolve `{LINKED_WORKTREE}` first** — `/do:pr` is routinely invoked from inside a `git worktree` (by `/do:next`, `/do:pr-better`, and the claim flows), and that changes which merge flags are safe:
     ...
     **Normalizing both paths is required, not tidiness**, and `cd … && pwd -P` is the normalization to use. Comparing the two `rev-parse` outputs raw misreports every ordinary repo entered from a subdirectory as a linked worktree: `--git-common-dir` prints a *relative* path there (`../.git`) while `--git-dir` prints an absolute one. `--path-format=absolute` would fix that but only on git ≥ 2.31 — older gits reject the flag, both substitutions come back **empty**, the two empty strings compare equal, and the probe fails *open* into `LINKED_WORKTREE=0`, restoring the exact `--delete-branch` breakage this check exists to prevent. `cd … && pwd -P` needs no flag support and resolves symlinks on both sides.
     ...
     Deleting through the **config-derived** remote and ref — never a hardcoded `origin` plus the local branch name — is the same rule the push step follows: a branch whose upstream is `upstream/feature-x` or `origin/pr-123-head` would otherwise lose an unrelated remote branch while the real PR head survives. The `MERGED` read-back matters just as much: `gh pr merge` exits zero on a repo with a **merge queue** while the PR is only queued, and GitHub auto-closes a PR whose head branch disappears, so an exit-status-only gate can kill the very merge it was waiting for. Whenever the merge is queued rather than done — a merge queue, or native `--auto` — **delete nothing**: those branches outlive this session, and the repo's "automatically delete head branches" setting or the caller's cleanup owns them.
...
6. After a **completed** merge, switch back and sync the default branch locally: `git checkout {default_branch} && git pull --rebase --autostash` — **only when `LINKED_WORKTREE=0`.** In a linked worktree that checkout is doomed for the same reason `--delete-branch` is (the parent repo holds `{default_branch}`), and there is nothing to sync here anyway: skip it, say so, and let the caller's cleanup phase sync the parent repo. When the merge is merely **queued** (native auto-merge, checks still running), skip the local sync too — the merge hasn't happened yet — and say so.
