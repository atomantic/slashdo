...
- **`--review-iterations <n>`**: cap how many review-and-fix cycles a **copilot** or **`@<login>`** pass runs per PR (Phase 6); no effect on `codex`/`agy`/`claude`/`grok`/`cursor`/`opencode`/`ollama` passes (their own fixed iteration caps). Set `REVIEW_ITERATIONS` from this value; default `1` (one review pass per PR, exiting early on 0 comments). `0` = loop until that reviewer returns 0 comments (legacy behavior, bounded by the 10-iteration guardrail). Must be a non-negative integer; otherwise abort with `--review-iterations must be a non-negative integer (got: {value}).` To move the local-agent / `ollama` caps — or to give each reviewer a different budget in one run — use the per-entry `--review-with <agent>~max=<n>` suffix, which overrides this flag for the entry that carries it.
...
The Swift-specific half of Phase 5b — run on `better-swift/{FIRST_CATEGORY}`
once the aggregate SemVer `{LEVEL}` has been determined:
...
**In issue mode Agent 7's findings are on disk, not in this context.** Phase 1 returned
...
- **Missing reviewer CLI** (`--review-with codex`/`agy`/`claude`/`grok`/`cursor`/`opencode` but the binary isn't installed): the multi-reviewer loop records that pass as `skipped` (→ inconclusive aggregate). It does NOT silently fall back to copilot
