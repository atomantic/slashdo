...
argument-hint: "[--interactive] [--scan-only] [--no-merge] [--review-iterations <n>] [path filter or focus areas]"
---
...
Pass each sub-agent the PR-specific variables: `{PR_NUMBER}`, `{OWNER}/{REPO}`, `better-swift/{CATEGORY_SLUG}`, `{BUILD_CMD}`, and `{REVIEW_ITERATIONS}` (the iteration cap; default 1).
