...
- **Missing reviewer CLI** (`--review-with codex`/`agy`/`claude`/`grok`/`cursor` but the binary isn't installed): the multi-reviewer loop records that pass as `skipped` (→ inconclusive aggregate). It does NOT silently fall back to copilot
