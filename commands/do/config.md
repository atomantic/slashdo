...
- **Auto-merge default for `/do:pr`** — `--merge` merges automatically once reviews and CI are solid; `--merge-method squash|rebase|merge` (or the shorthand `--merge=squash`) saves the method. Only `/do:pr` reads `merge`; `merge-method` is read by `/do:pr` and by `/do:next`, whose own merge always uses it.
