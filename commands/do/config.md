...
The store also holds per-agent default models (`--review-models`), the issue-mode default (`--issues`/`--issues-label`), `/do:next`'s claim gates (`--self`/`--collaborators`/`--trusted-authors`), and the auto-merge default (`--merge`/`--merge-method`) — see the flag-by-flag rules under "Recognized setting flags" below for what each key means, who reads it, and its built-in default; this section doesn't restate them.
