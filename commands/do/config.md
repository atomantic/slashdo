...
The store also holds per-agent default models (`--review-models`), the issue label (`--issues-label`), `/do:next`'s claim gates (`--self`/`--collaborators`/`--trusted-authors`), the auto-merge default (`--merge`/`--merge-method`), and the code-host / issue-tracker overrides (`--code-host`/`--tracker`) — see the flag-by-flag rules under "Recognized setting flags" below for what each key means, who reads it, and its built-in default; this section doesn't restate them.
...
!read lib/review-flags.md
   ...
   - `--tracker <github|gitlab>` → key `tracker`. Lowercase the value; it must be one of `github`, `gitlab`, else abort with `--tracker must be one of github, gitlab (got: {value}).` Selects the issue tracker independently of the code host (default: the code host itself). [lib/vcs-host.md](../../lib/vcs-host.md)'s tracker gate (`TRACKER_CLI`) treats a tracker with no backend for this repo's code host yet as **no tracker**: backlog commands (`/do:next`, `/do:replan`, `/do:plan-task`) stop, and commands that only defer findings list them in their report instead of filing. New tracker backends extend this value set and that gate together.
   - Any other `--flag` that is not one of the above and not `--show`/`--project`/`--reset`/`--unset` → abort with: `Unknown /do:config option: {flag}. Supported: --review-with, --review-models, --review-iterations, --review-mode, --reviewer-applies, --no-reviewer-applies, --review-stop-on-findings, --review-stop-on-clean, --review-stop-all, --issues-label, --self, --no-self, --collaborators, --no-collaborators, --trusted-authors, --merge, --no-merge, --merge-method, --code-host, --tracker, --unset <key>, --reset, --show, --project.`
4. **`--unset <key>`**: `<key>` must be one of `review-with`, `review-models`, `review-iterations`, `review-mode`, `reviewer-applies`, `review-stop-mode`, `issues` (removed key; unset only cleans up a stale value), `issues-label`, `self`, `collaborators`, `trusted-authors`, `merge`, `merge-method`, `code-host`, `tracker`. Reject others with `Unknown --unset key: {key}. Valid keys: review-with, review-models, review-iterations, review-mode, reviewer-applies, review-stop-mode, issues, issues-label, self, collaborators, trusted-authors, merge, merge-method, code-host, tracker.` (`--unset review-models` clears the entire per-agent map; to clear one agent, set an empty model with `--review-models <agent>=`.)
