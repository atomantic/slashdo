...
- `codex`, `claude`, `agy`, `grok`, `pi`, `cursor`, and `opencode` likewise accept a `<agent>[<model>]` bracket to pin the reviewing model — e.g. `codex[o3]`, `claude[claude-opus-4-8]`, `agy[Gemini 3.8 Flash (High)]`, `grok[grok-code-fast-1]`, `cursor[gpt-5]`, `opencode[muse-1.3]`. Strip the bracket into a per-entry `REVIEW_MODEL` (empty → the reviewer's built-in default) and keep the base slug. The bracket value is free-form (validate shape, not an allowlist); `copilot` and `@<login>` take no model bracket. A saved `review-models` default (see `/do:config`) supplies the model when a token omits the bracket — the bracket wins. See `lib/multi-reviewer-loop.md`.
