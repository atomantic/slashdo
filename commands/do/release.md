...
- This flag has no effect on local-agent reviewers (`codex`/`agy`/`claude`/`grok`/`pi`/`cursor`/`opencode`) or `ollama`; they keep their own fixed iteration caps. To move *their* caps — or to give each reviewer a different budget in one run — use the per-entry `--review-with <agent>~max=<n>` suffix, which overrides this flag for the entry that carries it. The `capped` verdict (an explicitly configured cap reached after applying fixes, from either source) counts as clean-equivalent for the merge gate — see the merge section below.
...
- **Generic promotion** — when no project release procedure is documented, continue
  at **Detect Release Workflow**. Its existing source-to-target promotion recipe
  and self-PR guard apply only to this path.
