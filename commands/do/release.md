...
3. **Pull latest target** — `git fetch origin {target} && git checkout {target} && git pull --rebase --autostash && git checkout {source}` — this ensures the local target branch matches origin before any diff or PR creation. Without this, the diff may be stale or include already-released changes.
