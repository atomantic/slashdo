...
- Accepted values per slot: `codex`, `agy` (aliases `gemini` / `antigravity` — all run the Antigravity CLI's `agy` binary), `claude`, `grok`, `ollama`, `copilot` (**legacy** — GitHub's cloud Copilot review; still supported when you name it, never selected implicitly), or an arbitrary GitHub login `@<login>`
...
- If any value is not in the accepted set, abort with a usage error: `Unknown --review-with value: {value}. Use one of: codex, agy, claude, grok, ollama, copilot, @<login> (each optionally suffixed ~opt and/or ~max=<n>).`
