...
## Mandate
Review each changed file, read in full, for per-file runtime correctness: code that crashes, fails, mishandles data, or produces wrong output. Siblings own the rest — quality, tests, and documentation drift (Surface Quality); call chains, state lifecycle, and concurrency across files (Cross-File Tracing); shapes, validation parity, and error classification (Cross-File Contract); secrets, auth, and injection (Security Audit). Drop a finding that fits a sibling's mandate rather than co-flagging it.
