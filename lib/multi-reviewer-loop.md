...
- `inconclusive` — the executed list contains at least one pass whose status is inconclusive (`timeout`, `error`, `guardrail`) AND zero passes returned `clean`. Reached when, e.g., `--review-with copilot` times out, the list exhausts, and no other reviewer ever produced a verdict. Distinct from `dirty` (build is fine) but still **not eligible to merge** — the user must re-run or intervene
- `partial` — some passes were skipped due to a stop-mode decision (`on-findings` or `on-clean` short-circuit) but the executed ones all completed normally (at least one `clean`)
- `clean` — every executed pass returned `clean` (or copilot `too-large`, which is treated as clean for merge purposes per the copilot loop's own rule) AND no hard-error short-circuit fired AND no inconclusive statuses remain
