...
2. **Validate each slug** is one of `copilot`, `codex`, `agy` (aliases `gemini` / `antigravity`), `claude`, `ollama` (optionally `ollama[<model>]`). Abort on the first unknown value with `Unknown --review-with value: {value}. Use one of: copilot, codex, agy, claude, ollama.` Normalize `gemini`/`antigravity` → `agy` before proceeding. For an `ollama[<model>]` entry, strip the bracket suffix into `{OLLAMA_MODEL}` for that entry and keep the base slug `ollama`; a bare `ollama` has an empty `{OLLAMA_MODEL}` (auto-select).
