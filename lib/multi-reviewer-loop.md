...
- `clean` — every executed pass returned `clean` (or copilot `too-large`, which is treated as clean for merge purposes per the copilot loop's own rule) AND no hard-error short-circuit fired
- `partial` — some passes were skipped due to a stop-mode decision (`on-findings` or `on-clean` short-circuit) but the executed ones all completed normally
...
The calling command (do:pr / do:release / do:review) uses `{OVERALL_STATUS}` to decide its own next action. For do:release in particular, the merge gate must require `{OVERALL_STATUS}=clean` — never merge on `dirty`, and on `partial` only when the stop-mode was explicitly set (i.e. the user opted into the short-circuit).
