...
For a normal audit, cover security, code quality, DRY, architecture, bugs/performance, stack-specific behavior, dependencies when third-party packages exist, and tests. Add UX only with `HAS_UI=true` and structural ambition only with `STRICT_MODE=true`. A caller-specific scope roster, when supplied, is authoritative; do not add generic scopes it intentionally omits. Run the test audit after the other selected scopes so it receives their compact finding index, not their full reports. Other scopes are independent.
