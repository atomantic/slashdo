...
## Principles
- Follow the data. For each externally influenced value, name its source, every sink it reaches (subprocess argv, file path, URL, query, prompt template, persisted authorization flag), and the sanitizer or validator at each boundary; a missing one is the finding.
