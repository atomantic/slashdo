...
- Adding a new exported function to a module that other test files MOCK requires updating those mocks. Strict mock-export checks (Vitest's strict mode, Jest `__esModule` strictness) fail every test that imports the router/module under that mock when the new symbol is missing from the mock object — the failure is blanket and misattributed to whatever the test asserts. Trace each new `export` → every `vi.mock('./that-module', ...)` / `jest.mock(...)` site and add the new symbol
...
- Tests whose name or description claims a behavior they don't actually assert (`'forwards metadata'` that only checks the title and author fields, ignoring the rest) lie about the contract — the test passes even when the named behavior regresses. Either rename the test to match what's asserted or add the missing assertion
