...
3. **Defer to the plan — only when the fix genuinely can't land here.** A finding qualifies for deferral ONLY when it is large or architectural (touches many subsystems, needs a design decision) OR carries real risk of breaking unrelated behavior if rushed into this change. When you defer, record it as a plan item with a one-line rationale for *why* it couldn't be fixed now. By default that means appending a `- [ ]` item to PLAN.md with a unique `[<slug>]` ID (per `~/.claude/lib/plan-id-format.md`). **If the command was invoked with `--issues`, file the deferred finding as a labeled tracker issue instead** — see `~/.claude/lib/plan-issue-mode.md`.
