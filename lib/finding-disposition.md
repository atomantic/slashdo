...
3. **Defer to PLAN.md — only when the fix genuinely can't land here.** A finding qualifies for deferral ONLY when it is large or architectural (touches many subsystems, needs a design decision) OR carries real risk of breaking unrelated behavior if rushed into this change. When you defer, append it to PLAN.md as a `- [ ]` item with a unique `[<slug>]` ID (per `~/.claude/lib/plan-id-format.md`) and a one-line rationale for *why* it couldn't be fixed now.
