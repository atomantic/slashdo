   ...
   **Runtime correctness**
   ...
   - Type coercion edge cases — `Number('')` is `0` not empty, `0` is falsy in truthy checks, `NaN` comparisons are always false, string `!==` doesn't do semantic ordering. Use explicit type checks (`Number.isFinite()`, `!= null`, semver libs) instead of truthy guards when zero/empty are valid values
