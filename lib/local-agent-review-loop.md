...
Verify prompt size before invocation: Linux caps a single argv string at 128 KiB (`MAX_ARG_STRLEN`), so on the argv paths (`agy`/`grok`/`cursor`/`opencode`/`pi`) check the prompt's **byte count**, not its character count, and record `STATUS=no-verdict` for oversized input (inconclusive, `~opt`-excusable). Never let `Argument list too long` become a hard `cli-error` that short-circuits later reviewers. Claude and `cmd` send the full payload on stdin and do not use this argv-size gate. Claude's separate stdin bound is below.

#### Claude stdin transport

The no-shell profile cannot reconstruct a Git patch itself. Use Claude's documented [piped input](https://code.claude.com/docs/en/cli-reference) instead: the CLI receives the complete `LOCAL_PROMPT` (review instructions, verdict contract, base/head IDs, changed-file scope, diff, and any needed context) through a private temporary file on stdin. No patch, file list, or source text is passed in argv, and no shell tool is added. The reviewer can use its existing read tools for source context. Never replace the patch with a summary or truncate it to fit.

Before **each Claude subprocess invocation**, after constructing the full payload, run this preparation block in the invocation's shell. It accepts at most **8 MiB (8,388,608 bytes)** of input, a transport safety bound rather than a promise about model context capacity. A missing/empty payload, failed write/count, or larger input is `STATUS=no-verdict`; return to the caller without launching the reviewer. Required reviewers remain unsatisfied; `~opt` only excuses inconclusive results, never turns them into clean reviews. If a verified CLI/provider has a smaller input limit, enforce that smaller limit as well; never silently accept incomplete coverage.
...
`mktemp` creates the input with owner-only permissions. Keep the file until the reviewer exits, including when the invocation runs in the background. After capturing its exit code, remove the input **before any success, timeout, CLI-error, or verdict-parser exit** (Step 2 shows the cleanup). Do not read the payload back into an argv string, `eval` it, or let the cleanup overwrite the captured reviewer exit code. An in-process Claude review uses the complete prompt directly through its Agent API and needs neither this file nor an argv-size check; the same enforced read-only tool requirement still applies.
...
<!-- /if:teams -->
   ...
   **`{INVOCATION}` resolves differently for `cmd`.** Its invocation alone must stay just `bash -c "$REVIEWER_CMD"` and its stdin pipe goes in front of the whole timed line: `printf '%s' "$LOCAL_PROMPT" | ${TIMEOUT_CMD[@]+"${TIMEOUT_CMD[@]}"} {INVOCATION}`. Putting the pipe inside `{INVOCATION}` instead (`{INVOCATION}` = `printf ... | bash -c ...`) would leave `TIMEOUT_CMD` wrapping only the `printf`, not the reviewer command — an unbounded `cmd` invocation with a timeout that times out nothing. `${MODEL_FLAG[@]+"${MODEL_FLAG[@]}"}`/`${EFFORT_FLAG[@]+"${EFFORT_FLAG[@]}"}` are omitted for `cmd` (neither is built for it). With an empty `TIMEOUT_CMD` (stock macOS) a `cmd` invocation has no outer bound at all — the operator's own invocation must supply one. **Claude also uses stdin**, but its invocation table row already redirects the prepared file, so it uses the ordinary (non-`cmd`) timed branch below. Run the Claude preparation block first and stop this pass on `STATUS=no-verdict`; do not put the payload back into argv. Both forms bound the reviewer process itself.
   ...
   - **Claude input cleanup** — after the foreground exit-code capture or background done-marker read, before handling any result, run:
     ```bash
