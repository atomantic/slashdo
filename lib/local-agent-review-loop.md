...
- **Validate the requested model first, exact match only.** A requested model must appear in the listing as an id or a display name. If it does, use it as-is — do not go on to the base-completion step below.
- **Otherwise, try completing it as a base.** Names in the listing are **leveled** (`Gemini 3.8 Flash (High)` / `gemini-3.8-flash-high`); agy itself rejects the bare base (`Gemini 3.8 Flash` / `gemini-3.8-flash`) if you passed it straight through. But a bare base is not automatically unrecognized: look for leveled entries that share it (`<base>-low|medium|high`, or `Base (Low|Medium|High)`). If any exist, this **is** a resolvable request — apply `{REVIEW_EFFORT}` by picking the sibling at the requested level (agy offers only `low`/`medium`/`high`, so `~effort=xhigh`/`max` take High and you say so — the intent is "as much reasoning as this reviewer has," not "abort"; no effort given picks the roster-default level, High). Report the completion in the run summary, e.g. `resolved agy[gemini-3.8-flash]~effort=low -> gemini-3.8-flash-low`. Do **not** fall through to the roster default in this case — the family survived, so the substitution below does not apply.
- **Roster default** (only when neither an exact match nor a same-base leveled sibling exists — a stale `AGY_REVIEW_MODEL`, a stale saved `review-models.agy`, a typo'd `agy[...]` bracket): the newest `* Flash` family at its highest level (e.g. `Gemini 3.8 Flash (High)` at the time of writing) — never a `Thinking`/`Pro` tier, which is what makes a review look hung. Fall back to it and say in the run summary which model you substituted and why.
