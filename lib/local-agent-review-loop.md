...
1. Confirm `{REVIEW_AGENT}` is one of `claude`, `codex`, `agy`, `grok` — the aliases `gemini` and `antigravity` normalize to `agy` (the Antigravity CLI's binary, successor to the Gemini CLI). Otherwise abort with a usage error. After this check, treat `{REVIEW_AGENT}` as the normalized value (`gemini`/`antigravity` → `agy`).
...
# codex / claude / grok: build the optional --model flag from the resolved {REVIEW_MODEL}.
...
# and expands to ZERO words when {REVIEW_MODEL} is empty (so codex/claude/grok fall
# back to the CLI's own default model — no flag passed; codex/grok both accept the
...
1. **Capture baseline**: `LOOP_START_SHA=$(git rev-parse HEAD)`
   ...
   **When `REVIEWER_APPLIES=false`, also snapshot the pre-review tree** so step 3 can enforce the read-only contract on the CLIs that cannot be sandboxed into it. `codex` is covered by `--sandbox read-only` (see the invocation table), but `claude`/`agy`/`grok` all run under `--dangerously-skip-permissions` / `bypassPermissions` — they have no read-only mode, so for them "do not touch the working tree" is only a *request in the prompt*, and a reviewer that decides to apply its own findings will silently rewrite the caller's files. Same four artifacts `lib/enhance-loop.md` uses, and for the same reason — **keep the two blocks in sync**:
   ```bash
...
3. **Detect changes and apply fixes** (logic depends on `{REVIEWER_APPLIES}`):
   ...
   **When `REVIEWER_APPLIES=false` (default — orchestrator applies)**:
   - **Enforce the read-only contract before reading the findings.** The CLI was *instructed* not to touch the working tree; verify that it didn't, because for `claude`/`agy`/`grok` nothing prevented it. Recompute all four step-1 artifacts and compare:
     ```bash
