...
| `codex` | `codex review --base "$BASE_BRANCH" --title "$REVIEW_TITLE"` | `codex exec -a never "$CODEX_APPLY_PROMPT"` |
...
- **codex (reviewer-applies)** uses `codex exec -a never "$CODEX_APPLY_PROMPT"` because `codex review` is read-only on the current shipped version (produces findings without modifying the working tree). `codex exec` accepts a free-form prompt that asks codex to review *and* apply fixes, with `-a never` selecting the never-ask approval mode so it runs unattended.
...
- `codex exec -a never` — top-level `codex` and `codex exec` commands accept `-a never` (never ask for approval; auto-approves all proposed actions). Only used in the reviewer-applies path; `codex review` rejects this flag.
