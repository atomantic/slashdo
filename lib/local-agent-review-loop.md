...
3. Confirm the CLI binary is installed: `command -v {REVIEW_BIN}`. If missing:
   ...
   - **`cmd` skips this step entirely** — there is no single binary name to probe; a broken or missing executable inside `{REVIEWER_CMD}` surfaces naturally as a non-zero exit in Step 2, recorded as `cli-error` there rather than `skipped` here.
...
**Reporting.** Since there is no fixed name, log and report it as `cmd:<first whitespace-delimited token of the invocation>` (e.g. `cmd[pi --harness ollama --model llama3]` reports as `cmd:pi`) — informative without spraying the full command line through every commit message and summary row.
...
<!-- /if:teams -->
   ...
   **`{INVOCATION}` resolves differently for `cmd`.** Every other agent takes `$LOCAL_PROMPT` as an argv string (`-p "$LOCAL_PROMPT"` or similar); `cmd` takes it on stdin instead (see "The `cmd` reviewer" above), so its `{INVOCATION}` in both code blocks below is `printf '%s' "$LOCAL_PROMPT" | bash -c "$REVIEWER_CMD"` — the whole pipeline, not just the trailing command — and `${MODEL_FLAG[@]+"${MODEL_FLAG[@]}"}`/`${EFFORT_FLAG[@]+"${EFFORT_FLAG[@]}"}` are omitted (neither is built for `cmd`).
