...
3. **Parse findings and apply fixes** (orchestrator-applies — the same flow as the local-agent loop's review-only path):
   - Split `$LOG_FILE` on the delimiter regex `^===== FILE: (.+) =====$` and JSON-parse each section, collecting every entry across valid `findings` arrays (see the defensive-parsing note above). For every malformed section increment `PARSE_ERRORS`; if `REVIEW_ERRORS + PARSE_ERRORS == REVIEWABLE`, set `STATUS=cli-error` and exit because no file yielded a usable verdict. If there are no findings, set `STATUS=clean` — **but if there was any coverage gap (`REVIEW_ERRORS + PARSE_ERRORS + TRUNCATED > 0`), set `STATUS=incomplete` instead** (the diff was only partially reviewed, so "no findings" is not a clean verdict) — and exit the loop.
