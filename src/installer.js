...
function mutateSettings(env, hookFiles, dryRun, adapterName) {
  const adapter = env[adapterName];
  if (!env.settingsFile || typeof adapter !== 'function') return [];

  const settingsPath = env.settingsFile;
  if (!fs.existsSync(settingsPath) && adapterName === 'deregisterHooks') return [];
  ...
  if (fs.existsSync(settingsPath)) {
    try {
      ...
      return [{ name: 'settings.json', status: 'skipped (parse error)' }];
    }
  }

  const { actions, modified } = adapter(settings, hookFiles, env, dryRun);
  if (!dryRun && modified) fs.writeFileSync(settingsPath, JSON.stringify(settings, null, 2) + '\n', 'utf8');
  return actions;
}
...
function install({ env, packageDir, filterNames, dryRun, uninstall, autoUpdate }) {
  ...
  if (env.supportsHooks && env.hooksDir) {
    ...
    if (!filterNames?.length) {
      const settingsActions = mutateSettings(env, hookFiles, dryRun, 'registerHooks');
...
function doUninstall(commands, libFiles, hookFiles, env, results, dryRun, filterNames) {
  ...
  if (env.supportsHooks && env.hooksDir && !filterNames?.length) {
    ...
    const settingsActions = mutateSettings(env, hookFiles, dryRun, 'deregisterHooks');
    results.actions.push(...settingsActions);
