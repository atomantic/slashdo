...
**How much of the dispatch fires depends on your host.** Both labels are always written, always filterable, and always reported. What varies is the dispatch step: a host applies each axis only if its sub-agent API exposes that control. Under **Claude Code today, `model:` dispatches and `effort:` does not** — its `Agent` tool takes a model but no effort parameter (reasoning effort comes from an agent's definition, not the dispatch call), so `effort:` acts as a filter and a recorded signal there rather than a per-agent setting, and each worker runs at the session's effort.
