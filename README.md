...
**One asymmetry worth knowing:** `--issues-label` *scopes* the commands that **file or triage** plan items (replan triages only labeled issues; better/depfree/review file under the label), but `/do:next` — which **consumes** the queue — is **label-agnostic by default**. A bare `/do:next --issues` claims the oldest open issue regardless of label (skipping only parking labels like `future`/`epic`/`blocked` and anything already in flight or assigned), so a repo that files ordinary `enhancement`/`bug`/`area:*` issues works without first stamping a `plan` label on everything. Pass `/do:next --issues --issues-label <name>` (or save the default) to restrict its auto-pick to a curated label.
