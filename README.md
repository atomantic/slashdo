...
**Drain the backlog in parallel — `/do:next --issues --swarm`.** By default `/do:next` ships one item per run. With `--swarm` (issues mode) it claims and ships **several independent open issues at once**, each in its own worktree subagent running the normal single-issue flow, then **serializes only the merge**. It picks the first N independent issues off the same priority/oldest queue (skipping any that depend on or obviously overlap another in the batch), fans out one agent per issue to implement + open a reviewed PR, then merges the PRs one at a time — re-syncing each onto the advancing default branch (deletions-win on changelog/PLAN conflicts) and routing any flaky-CI failure through the same one-re-run guard the merge gate uses. `--swarm` bare runs 3 agents; `--swarm=N` sets the count (clamped `1..6`, since N agents cost ≈N× the tokens). A PR whose review gate isn't clean, or that can't cleanly re-sync, is left open for you rather than force-merged; a died agent's claim is released back to the queue. It's the throughput multiplier over running `/do:next` one item at a time.
