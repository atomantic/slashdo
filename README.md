...
|:---|:---|:---|
| `--review-with <agent>[,<agent>...]` | empty — **no default reviewer** (except `/do:rpr`, see below) | Pick one or more reviewers, run in the order given. Omit the flag and no external review runs (each command still runs its own unconditional self-review gate). Accepted slugs: `copilot` (GitHub cloud review), `codex`, `agy` (aliases `gemini` / `antigravity` — all run the Antigravity CLI's `agy` binary), `claude` (each non-copilot slug spawns that local CLI in headless mode), `ollama` (review with a local Ollama model — bare `ollama` auto-selects the most capable installed coding model; `ollama[<model>]` pins a specific installed model, e.g. `--review-with=ollama[qwen2.5-coder:32b]`; the same `<agent>[<model>]` bracket pins the model for `codex`, `claude`, and `agy` too — e.g. `--review-with=claude[claude-opus-4-8],codex[o3]` — or save a per-reviewer default with `/do:config --review-models` so runs can omit the bracket), and `@<login>` (request a review from an **arbitrary GitHub reviewer** — any user or App/bot, e.g. `@octocat`, `@org-review-bot`, `@some-app[bot]`: slashdo requests their review on the PR, waits for it, and fixes what it surfaces, just like the `copilot` pass — GitHub only, and slashdo never posts an approval itself. **Not supported by `/do:rpr`**: it accepts the syntax but drops `@<login>` entries with a notice and falls back to its own conditional `copilot` default instead, since rpr runs a bespoke Copilot-only request/monitor flow rather than the multi-reviewer loop). Whatever you list is exactly what runs — `--review-with codex` runs codex only; copilot is never added implicitly. Example: `--review-with codex,agy,copilot` runs codex first, then Antigravity, then copilot, each reviewing the branch as the previous pass left it. **Optional (non-blocking) reviewers:** suffix any slot with `~opt` — e.g. `--review-with=claude,ollama~opt,codex` or `--review-with=codex,@flaky-bot~opt` — to keep that reviewer running and fixing findings while excluding its *inconclusive* result (timeout / skipped / no-verdict) from the merge gate, so it never blocks `--merge`. A hard-error (broken build / failed tests) from an optional reviewer still blocks. Use it for a second-opinion reviewer that doesn't reliably return a verdict, such as a local Ollama model. On `/do:better`, `/do:better-swift`, and `/do:depfree`, omitting the flag means the review loop **and the auto-merge** are skipped — PRs are left open for manual review. |
| `--review-iterations <n>` | `1` | Cap how many review-and-fix cycles a **copilot** or **`@<login>`** pass runs. Default `1`: request one review, apply every fix it surfaces, then stop (exiting early if the review returns 0 comments). `0` restores the legacy "loop until 0 comments" behavior, bounded by a 10-iteration safety guardrail. No effect on `codex`/`agy`/`claude` passes (fixed 3-iteration cap) or `ollama` (its own fixed cap), and no effect when neither `copilot` nor `@<login>` is in the list. Accepted by `/do:better`, `/do:better-swift`, and `/do:depfree` too. |
...
| `--reviewer-applies` | off | Edit the working tree directly from the reviewing CLI instead of routing findings back through the orchestrating thread. No effect on the GitHub-side passes — `copilot` and `@<login>` (both review read-only cloud-side) — or ollama passes (Ollama is non-agentic — always review-only); takes effect on each codex / agy / claude pass in the list. |
...
You can also pin **which model each reviewer runs on** — for the model-taking reviewers `codex`, `claude`, `agy`, and `ollama` — either per run via the `<agent>[<model>]` bracket (`--review-with=claude[claude-opus-4-8],codex[o3]`) or as a saved default so future runs can omit the bracket: `/do:config --review-models codex=o3,claude=claude-opus-4-8`. Set one agent at a time (the others are left intact), and clear one with an empty value (`--review-models codex=`). Precedence per reviewer: an explicit `<agent>[<model>]` bracket (whether typed on the run or carried by a saved `review-with` default) → a per-project saved model → a global saved model → the reviewer's built-in default (the codex/claude CLI default, agy's pinned default, or ollama's auto-select). The GitHub-side reviewers `copilot` and `@<login>` take no model.
...
| `/do:config --review-models <agent>=<model>,…` | Save the default model per reviewer (`codex`/`claude`/`agy`/`ollama`); merges key-by-key, an empty value clears one agent |
...
| `--unset <key>` | Clear one saved default (`review-with`, `review-models`, `review-iterations`, `reviewer-applies`, `review-stop-mode`, `issues`, `issues-label`, `self`, `merge`, `merge-method`) |
