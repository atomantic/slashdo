...
rewrite_for_opencode() {
  sed -e "$REQUIRED_READ_REWRITE" \
      ...
      -e 's|~/.claude/commands/do/\([A-Za-z0-9._-][A-Za-z0-9._-]*\)\.md|~/.config/opencode/commands/do-\1.md|g' \
...
LIBS=(
  ...
  local-agent-review-loop merge-gate model-tiers multi-reviewer-loop multi-reviewer-parallel next-gitlab next-swarm ollama-review-loop
