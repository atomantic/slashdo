...
- **CI publish workflow switched to npm OIDC trusted publishing.** The `Release` workflow no longer references the `NPM_TOKEN` secret; `npm publish --provenance` authenticates via GitHub OIDC instead. Node is pinned to v24 in the workflow so that the bundled npm (≥ 11.5.1) supports trusted publishing.
...
**Full Diff**: https://github.com/atomantic/slashdo/compare/v2.18.0...v2.19.0
